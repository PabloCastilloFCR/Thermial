...
void setPWM(int pin, int value) {
    ...
    uint slice_num = pwm_gpio_to_slice_num(pin); // Obtener el canal PWM
    pwm_set_wrap(slice_num, 255); // PWM de 8 bits (0-255)
    pwm_set_gpio_level(pin, value); // Configurar valor PWM
    pwm_set_enabled(slice_num, true); // activar PWM
...
void setup() {
    ...
    delay(1000);
    Serial.begin(115200);
    Serial.println("Test: Código 0x10 corre!");    
...
int get_function(){ 
    return (int)(flowRate * 100); // código del flujometro o de la bomba
}
...
void requestEvent() {
    uint16_t flowData = static_cast<uint16_t>(get_function());  // datos de flujo (flowRate * 100)
